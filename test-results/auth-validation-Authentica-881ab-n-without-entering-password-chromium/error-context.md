...
- Name: auth-validation.spec.ts >> Authentication Form Validation Tests >> TC-AUTH-FE-004: User attempts login without entering password
- Location: tests\auth-validation.spec.ts:10:7
...
```ts
  ...
  2   | 
  3   | test.describe('Authentication Form Validation Tests', () => {
  4   |   test.beforeEach(async ({ page }) => {
  5   |     // Clear localStorage and cookies before each test
