...
export default function HistoryPage() {
  ...
  useEffect(() => {
    ...
    const mock = [
      { id: '1', model: 'GPT-5', date: '2025-04-01', messages: 12, preview: 'Help me find the best AI model for my project' },
      { id: '2', model: 'Claude Opus 4.5', date: '2025-03-31', messages: 8, preview: 'Analyse documents and extract key information' },
      { id: '3', model: 'Gemini 2.0 Pro', date: '2025-03-30', messages: 5, preview: 'Build a quick prototype for my app idea' },
    ];
    setSessions(mock);
  }, [router]);
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {sessions.map(s => (
              <div key={s.id} style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text)', marginBottom: '0.25rem' }}>{s.preview}</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text3)' }}>{s.model} · {s.messages} messages · {s.date}</div>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <button onClick={() => setSessions(p => p.filter(x => x.id !== s.id))} style={{ padding: '0.4rem 0.9rem', background: 'none', border: '1px solid var(--border2)', borderRadius: 'var(--radius-sm)', fontSize: '0.78rem', color: 'var(--text3)', cursor: 'pointer', fontFamily: 'inherit' }}>Delete</button>
                </div>
              </div>
            ))}
          </div>
